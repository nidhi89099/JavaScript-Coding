...
/*function solve(string) {
  ...
  return s.map(m => m[0].toUpperCase() + m.slice(1)).join(' ')
  //.map(s => s.toUpperCase())
}
console.log(solve("How can mirrors be real if our eyes aren't real"))*/

function solve(string) {
  const s = string.split(' ')
  return s.map(m => m.replace(m[0], m[0].toUpperCase())).join(' ')
